...
test('latest-replaces: second submit removes the prior set, reclaims quota, keeps one row/slot', () => {
  ...
  const files = database.prepare("SELECT name FROM nodes WHERE parent_id=? AND kind='file'").all(sub) as {
    name: string;
  }[];
...
test('over quota on REPLACE rolls back and preserves the prior response', () => {
  ...
  const files = database.prepare("SELECT name, size_bytes FROM nodes WHERE parent_id=? AND kind='file'").all(sub) as {
    name: string;
    size_bytes: number;
  }[];
...
test('responseHeadroom: unlimited -> null; bounded accounts for the prior set', () => {
  ...
  const otherDept = Number(
    database
      .prepare(`INSERT INTO collection_departments(collection_id,name,position,created_at) VALUES (?, 'Finance', 1, ?)`)
      .run(c2.collectionId, NOW).lastInsertRowid,
  );
